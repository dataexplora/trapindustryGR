...
interface ArtistReactionButtonsProps {
  artistId: string;
}

const ArtistReactionButtons: React.FC<ArtistReactionButtonsProps> = ({ artistId }) => {
  ...
  const handleReactionClick = async (reactionType: keyof ReactionCounts) => {
    try {
  ...
  const renderReactionButton = (type: keyof ReactionCounts, icon: React.ReactNode, label: string) => {
    ...
    return (
      ...
      >
        <div className={`mb-1 ${hasReacted ? 'text-indigo-400' : 'text-gray-300'}`}>
          {icon}
        </div>
  ...
  if (isLoading) {
    return <div className="flex justify-center py-4 text-gray-400">Loading reactions...</div>;
  }

  return (
    <div className="relative flex flex-col space-y-4">
      <h3 className="text-sm text-gray-400 font-medium">React to this artist:</h3>
      <div className="flex justify-around items-center bg-dark-card rounded-lg p-2">
      ...
      {/* Rate limit popup */}
      {showRateLimitPopup && (
        <div className="absolute top-16 left-1/2 transform -translate-x-1/2 bg-red-600 text-white px-4 py-2 rounded-lg shadow-lg z-10 whitespace-nowrap animate-bounce">
      ...
      {rateLimit && (
        <div className="text-xs text-amber-500 flex items-center">
          ...
          You've reached the reaction limit (3/hour).
        </div>
      )}
    </div>
  );
...
export default ArtistReactionButtons; 